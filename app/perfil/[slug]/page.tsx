...
function buildFallbackArticle(p: any) {
  ...
  return {
    hook: "Houve um tempo em que olhar para as finanças significava apertar o peito — até eu descobrir que organização é um ato de gentileza com o meu futuro.",
    content: [
      {
        type: "paragraph",
        text:
`Lembro do domingo em que decidi trocar a culpa por curiosidade. Abri meu aplicativo do banco com um café do lado e me perguntei: o que esse extrato está tentando me contar sobre mim? Foi a primeira vez que dinheiro virou linguagem — não sentença.`
      },
      {
        type: "paragraph",
        text:
`Comecei pequeno: 30 minutos semanais para revisar a última semana e desenhar a próxima. Nada de planilhas mirabolantes — só uma bússola que coubesse na vida real. O resultado? Menos ansiedade, mais clareza, e a liberdade de dizer “não” ao que não conversa com meus valores.`
      },
      {
        type: "tips",
        items: [
      ...
      },
      {
        type: "paragraph",
        text:
`Com ${name}, o papo sobre ${p?.sector ?? "finanças"} é menos sobre fórmulas e mais sobre escolhas consistentes. ${set.map((t:string)=>`#${t}`).join(" ")}`
      },
      {
        type: "cta",
    ...
    <div className="relative min-h-screen">
      ...
      <div className="container mt-6">
        <div className="relative">
          {/* Banner visual (continua igual) */}
          <WhatsAppBanner
            phone="5511987654321"
            title={`Fale com ${p.display_name} no WhatsApp`}
            subtitle="Consultas, parcerias e convites"
            text={`Oi, ${p.display_name}! Encontrei seu perfil no LUMINA e gostaria de conversar.`}
          />
          {/* Overlay que captura o clique e leva ao assistente */}
          <a
            href={`/assistente/${encodeURIComponent(slug)}`}
            className="absolute inset-0"
            aria-label={`Abrir assistente de ${p.display_name}`}
          />
        </div>
      </div>
        ...
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
            ...
            <div className="min-w-0 space-y-10">
              ...
              <AdSlot id="ad-rectangle-mid" label="Publicidade" size="300x250" variant="leaderboard" />
